...
public class Login3rdAction extends LoginAction {
	...
	public String login(HttpServletRequest request,HttpServletResponse response, RepInfo repInfo) {
		...
		try {
			...
			LOG.info("accept third login param="+param);
			...
			LOG.info("accept third login userInfo="+userInfo);
...
//            	}
            	...
            	if(null==sessionId){
            		...
            		keepUserCookieSession(request, response, userInfo, type, "1","-1",repInfo);
            	}else{
            		keepUserCookieSession(request, response, userInfo, type, "1",sessionId,repInfo);
            	}
...
//                userInfo.put("login", "1");
        ...
        long userTime=System.currentTimeMillis()-loginBegin;
        if(userTime>3000){ //将请求大于三秒的请求输出
        	LOG.info("accept third login timeout the result ="+result);
        }
		return result;
	}
...
//			accessTokenInfo.put("unionid", "o4XC4t2s_JVZNUPythwY-gkzleXk");
			...
			LOG.info("accept third login wechat accessTokenInfo="+accessTokenInfo.toJSONString());
			// 检测是否取到unionid
			if (null != accessTokenInfo
				...
				if (null == userMap) {
					...
					LOG.info("accept third login wechat jsonUser="+jsonUser.toJSONString());
					if (thirdLoginInfo.getFrom().contains("aiuw")) {
						jsonUser.put("from", 1);// 设置来源aiuw
					}
	...
	public String keepUserCookieSession(HttpServletRequest request,
			...
			String loginFrom, String isAutoLogin,String sessionId,RepInfo repInfo) {
		String userId = userInfo.get("userID");
		String userName = userInfo.get("userName");
		...
		try {
            ...
            uid.setPath("/");
            name.setPath("/");
            if(StringUtils.isNotEmpty(repInfo.getHostPrefix())){
                uid.setDomain(repInfo.getHostPrefix());
                name.setDomain(repInfo.getHostPrefix());
            }
    		...
    		if (com.meiqi.openservice.commons.config.Constants.UserType.HEMEIJU_USER
    			...
    			LOG.info("third login mergeCart:sessionId="+sessionId+",userId="+userId);
    			// 合并购物车
            ...
            e.printStackTrace();
            LOG.info("Login3rdAction:"+e.getMessage());
        }
		return DsResponseCodeData.ERROR.code;
	}
