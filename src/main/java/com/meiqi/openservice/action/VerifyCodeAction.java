...
public class VerifyCodeAction extends BaseAction
{
    ...
    public void getVerifyCode(HttpServletRequest req, HttpServletResponse resp,RepInfo repInfo)
    {
        ...
        String verifyCodeType = Constants.NormalVerifyCodeType.get(codeType);
        if (StringUtils.isBlank(verifyCodeType)) {
            LogUtil.error("VerifyCode code note defined, codeType:"+codeType);
            return;
        }
        req.getSession().setAttribute(verifyCodeType, randomCode.toString());
        // 禁止图像缓存。
