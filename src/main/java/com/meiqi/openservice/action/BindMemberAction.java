...
public class BindMemberAction extends BaseAction{
	 ...
	 * @return
	 */
	private String addInviteCode(Map<String,Object> set,int i,String operateResult){
		List<Action> newActions=new ArrayList<Action>();
		Map<String,Object> paramMap1 = new HashMap<String, Object>();
		Action action = new Action();
	 ...
	 * @return
	 */
	public String designerJoin(String user_id){
		String resultData = "";
		ResponseInfo respInfo = new ResponseInfo();
		DsManageReqInfo dsManageReqInfo1 = new DsManageReqInfo();
		Map<String, Object> param1 = new HashMap<String, Object>();
		...
		Map<String,Object> paramMap1 = new HashMap<String, Object>();
		dsManageReqInfo1.setNeedAll("1");
		dsManageReqInfo1.setServiceName("COM_HSV1_JoinReward");
		param1.put("user_id", user_id);
		dsManageReqInfo1.setParam(param1);
		
		resultData = dataAction.getData(dsManageReqInfo1, "");
		LogUtil.info("BindMember COM_HSV1_JoinReward param is: "+user_id);
		LogUtil.info("BindMember COM_HSV1_JoinReward result is: "+resultData);
		RuleServiceResponseData responseData1 = null;
		List<Map<String,String>> mapList1= new ArrayList<Map<String,String>>();
		responseData1 = DataUtil.parse(resultData, RuleServiceResponseData.class);
		
		if (!DsResponseCodeData.SUCCESS.code.equals(responseData1.getCode())) {
			LogUtil.error("BindMember COM_HSV1_JoinReward param is: "+user_id);
			LogUtil.error("BindMember COM_HSV1_JoinReward return is: "+resultData);
			respInfo.setCode(DsResponseCodeData.ERROR.code);
			respInfo.setDescription("入驻奖励查询失败！");
			return JSON.toJSONString(respInfo);
	    }
		...
		if(mapList1 != null && mapList1.size() > 0){
			int com_money = 0;
			if(!StringUtils.isEmpty(mapList1.get(0).get("com_money"))){
				com_money = Integer.parseInt(mapList1.get(0).get("com_money"));
			}
			int statues = Integer.parseInt(mapList1.get(0).get("statues"));
			int frm_id = Integer.parseInt(mapList1.get(0).get("frm_id"));
			if(statues != 0 && com_money > 0 ){
				String commission_sn = "JS"+getNumber();
				Action action = new Action();
				...
				setMap.put("settlement_status", 0);
				setMap.put("frm_id", frm_id);
				setMap.put("adjust_amount", com_money);
				setMap.put("commission_type", 2);
				setMap.put("create_user", "admin");
				setMap.put("create_time", "$UnixTime");
				setMap.put("commission_sn", commission_sn);
				action.setSet(setMap);
				newActions.add(action);
				
				Action action1 = new Action();
				action1.setServiceName("test_ecshop_ecs_adjust_info");
				Map<String, Object> setMap1 = new HashMap<String, Object>();
				action1.setType("C");
				setMap1.put("user_id", user_id);
				setMap1.put("user_type", 2);
				setMap1.put("adjust_reason", "入驻奖励");
				setMap1.put("settlement_status", 1);
				setMap1.put("adjust_amount", com_money);
				setMap1.put("relate_bill", commission_sn);
				setMap1.put("creat_name", "admin");
				setMap1.put("create_time", "$UnixTime");
				setMap1.put("adjust_sn", "TZD"+getNumber());
				action1.setSet(setMap1);
				newActions.add(action1);
				
				Action action2 = new Action();
				action2.setServiceName("test_ecshop_ecs_audit_detail");
				Map<String, Object> setMap2 = new HashMap<String, Object>();
				action2.setType("C");
				setMap2.put("frm_id", frm_id);
				setMap2.put("audit_type", 0);
				setMap2.put("commission_id", "$-2.generateKey");
				action2.setSet(setMap2);
				newActions.add(action2);
				
				Action action3 = new Action();
				action3.setServiceName("test_ecshop_ecs_action_log");
				Map<String, Object> setMap3 = new HashMap<String, Object>();
				action3.setType("C");
				setMap3.put("action_user", "admin");
				setMap3.put("action_time", "$UnixTime");
				setMap3.put("action_content", "对"+user_id+"生成了一条入驻奖励结算单");
				setMap3.put("type", 16);
				setMap3.put("bill_id", "$-3.generateKey");
				...
				JSONObject job = JSONObject.parseObject(result);
				if (!DsResponseCodeData.SUCCESS.code.equals(job.get("code"))) {
					LogUtil.error("BindMember designerJoin param is:"+dsReqInfo);
					LogUtil.error("BindMember designerJoin result is:"+result);
					respInfo.setCode(DsResponseCodeData.ERROR.code);
	                respInfo.setDescription("入驻奖励失败！");
	                return JSON.toJSONString(respInfo);
			    }
				return result;
			}
