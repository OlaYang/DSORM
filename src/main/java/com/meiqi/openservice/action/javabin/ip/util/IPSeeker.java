...
public class IPSeeker {
	...
	public static Map<String, IPLocation> ipCache;
	// 随机文件访问类
	public static RandomAccessFile ipFile;
	// 内存映射文件
	...
	public static void loadIpFile(){
        ...
        String dataPath="";
        try {
            dataPath= BaseAction.basePath + File.separator + "lejj_resource" + File.separator + "ip"+ File.separator +"qqwry.dat";
            File path = new File(dataPath);
            ipFile= new RandomAccessFile(path, "r");
        } catch (FileNotFoundException e) {
            ipFile=null;
            LogUtil.error("ip data file  error dataPath:"+dataPath+",error:"+e.getMessage());
        }
        // 如果打开文件成功，读取文件头信息
        if (ipFile != null) {
            try {
...
//	}
	...
	private IPLocation getIPLocation(long offset) {
		...
		try {
		    ...
		    LogUtil.error("ip error:"+e.getMessage());
		    if(ipFile==null){
		        loadIpFile();
		    }
			return null;
