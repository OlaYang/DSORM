...
//        String discountCode = order.getDiscountCode();  ipad传入折扣吗为code 杨永川使用discountcode 导致一直无法取得折扣吗
        ...
        String orderSn=com.meiqi.openservice.commons.util.StringUtils.getNumber();//17位
    ...
    private List<Order> dataToObject(List<Map<String,String>> rows){
    	...
    	for(int i=0;i<rowsSize;i++){
    		...
    		order.setOrderStatusInfo(map.get("order_status_info"));
    		orders.add(order);
    ...
    private String assembleOrderListBySchema(List<Order> allOrderList, String plat, int orderTotal) {
        ...
        for (Order order : allOrderList) {
            ...
            String xmlName = getXmlNameByOrderStatusInfo(ORDER_LIST, orderStatusInfo, orderGoodsList.size(), plat);
    ...
    /*private String getXmlNameByOrderStatus(String xmlType, byte orderStatus, byte payStatus, byte shippingStatus,
        ...
        LogUtil.info("order xml xmlName:"+xmlName);
        return xmlName.toString();
    }*/
    private String getXmlNameByOrderStatusInfo(String xmlType, String orderStatusInfo, int goodsSize, String plat) {
        LogUtil.info("getXmlNameByOrderStatusInfo - xmlType:"+xmlType+" orderStatusInfo:"+orderStatusInfo+" goodsSize:"+goodsSize+" plat:"+plat);
        String transactionStatus = "";
        StringBuffer xmlName = new StringBuffer();
        xmlName.append(XML_PATH).append(xmlType).append("/").append(xmlType + "_");
        // app App订单状态 等待支付
        ...
        /*
                               规则返回状态有：待付款 、付款中 、待发货 、部分发货 、已收货 、已取消 、待定 
                                但app 中只定义了 待付款 、待发货 、关闭 、退货 、发货中 、交易成功
         ...
         app 中的退货暂时不出现。
         */
        if ("待付款".equals(orderStatusInfo)) {
            transactionStatus = "waitPay_";
        } else if ("付款中".equals(orderStatusInfo)) {
            transactionStatus = "waitPay_";
        } else if ("待发货".equals(orderStatusInfo)) {
            transactionStatus = "waitShipping_";
        } else if ("部分发货".equals(orderStatusInfo)) {
            transactionStatus = "shipping_";
        } else if ("已收货".equals(orderStatusInfo)) {
            transactionStatus = "shippingSucceed_";
        } else if ("已取消".equals(orderStatusInfo)) {
            transactionStatus = "dealClosed_";
        } else if ("待定".equals(orderStatusInfo)) {
            transactionStatus = "dealClosed_";
        } else {
            LOG.error("getXmlNameByOrderStatusInfo - unidentified orderStatusInfo：" + orderStatusInfo);
        }
        ...
        xmlName.append(plat).append(".xml");
        LogUtil.info("getXmlNameByOrderStatusInfo - xmlName:"+xmlName);
        return xmlName.toString();
    }
    ...
    private String assembleOrderDetailBySchema(Order order, String plat) {
        ...
        String xmlName = getXmlNameByOrderStatusInfo(ORDER_DETAIL, orderStatusInfo, orderGoodsList.size(), plat);
        // 获取root docment
        Document document = XmlUtils.createDocument();
    ...
    public String getOrderDetail(long userId, long orderId, String plat) {
        DsManageReqInfo dsManageReqInfo=new DsManageReqInfo();
        dsManageReqInfo.setServiceName("HMJ_HSV1_MyOrder");
        dsManageReqInfo.setNeedAll("1");
        Map<String,Object> param=new HashMap<String, Object>();
        ...
        List<Map<String, String>> rows=responseData.getRows();
        List<Order> allOrderList=dataToObject(rows);
        if (null == allOrderList || 1 != allOrderList.size()) {
            LOG.error("getOrderDetail - get order fail");
            return null;
        }
        ...
        return assembleOrderDetailBySchema(order, plat);
    }
     ...
     */
    @Override
    public String getAllOrderByPhone(String phone, int pageIndex, int pageSize, String plat) throws Exception {
        String result = "";
        DsManageReqInfo dsManageReqInfo = new DsManageReqInfo();
        dsManageReqInfo.setServiceName("IPAD_BUV1_Myordercount");
        dsManageReqInfo.setNeedAll("1");
        ...
        dsManageReqInfo.setParam(param);
        RuleServiceResponseData responseData = null;
        String totalRes = dataAction.getData(dsManageReqInfo, "");
        responseData = DataUtil.parse(totalRes, RuleServiceResponseData.class);
        if (!DsResponseCodeData.SUCCESS.code.equals(responseData.getCode())) {
            LOG.error("getAllOrderByPhone - IPAD_BUV1_Myordercount err:" + responseData.getDescription());
            throw new Exception(responseData.getDescription());
        }
        List<Map<String, String>> rows = responseData.getRows();
        int orderTotal = 0;
        if (1 == rows.size()) {
        ...
        if (orderTotal > 0) {
            ...
            responseData = DataUtil.parse(resultData, RuleServiceResponseData.class);
            if (!DsResponseCodeData.SUCCESS.code.equals(responseData.getCode())) {
                LOG.error("getAllOrderByPhone - HMJ_HSV1_MyOrder err:" + responseData.getDescription());
                throw new Exception(responseData.getDescription());
            }
            rows = responseData.getRows();
            ...
            List<Order> allOrderList = dataToObject(rows);
            if (null == allOrderList || orderTotal != allOrderList.size()) {
                LOG.error("getAllOrderByPhone - orderTotal and orderList size not equal!");
                throw new Exception("orderTotal and orderList size not equal");
            }
