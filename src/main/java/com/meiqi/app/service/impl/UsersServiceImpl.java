...
public class UsersServiceImpl implements UsersService {
     ...
     */
    @Override
    public boolean isValidCode(String phone, String code, byte type, boolean setInvalid) {
        if (setInvalid) {
            return validateSmsCode(phone, code, type);
        } else {
            return isValidCode(phone, code, type);
        }
    }
    
    private boolean validateSmsCode(String phone, String code, byte type) {
        ...
        Object obj = smsAction.validateSmsCode(null, null, req);
    ...
    public Users isValidCode(Users user) {
       ...
       /* Users userForDB = usersDao.getUsersByUserName(cls, user.getPhone());
        if (null != userForDB) {
            result = isValidCode(userForDB.getPhone(), user.getCode(), user.getType(), true);
            if (result) {
                return userForDB;
            }
        }*/
        ...
        result = isValidCode(mapList.get(0).get("user_name"), user.getCode(), user.getType(), true);
        if (result) {
