...
public class SolrClientUtil {
    ...
    static Pattern englishRangePattern = Pattern.compile("^\\*[a-zA-Z]+\\*$");
    private static final String URL_ENCODE = "utf-8";
    private static final String wt = "json";
    private static final String meiqisolrsplit="solrsplit";//分隔符
...
//        return client;
    ...
    * @throws
     ...
     * @throws 
     */
     private static String buildQQuery(String query) {
         StringBuffer sb=new StringBuffer();
         try {
            Map<String,String> map=DataUtil.parse(query,Map.class);
            for(String key:map.keySet()){
                String value=map.get(key);
                String[] array=value.split(meiqisolrsplit);
   ...
   //判断表示是否全为英文  
     public static boolean strIsEnglish(String word) {  
         Matcher m = englishRangePattern.matcher(word);
         return m.find();
     }  
        ...
        if (objJson instanceof net.sf.json.JSONArray) {
            ...
            while (it.hasNext()) {
                ...
                if (object instanceof net.sf.json.JSONArray) {
                    ...
                    String[] array = object.toString().split(meiqisolrsplit);
                    for (String str : array) {
                        if (!isRangeExp(str)) {
                            if(strIsEnglish(str)){
                                str = str.toLowerCase();
