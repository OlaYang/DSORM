...
public abstract class AbstractWeChatService {
	...
	protected BaseMsg invokeService(BaseReq msg, String eventKey, Map<String, Object> reqMap) {
		...
		try {
			...
			Map<String, Object> param = new HashMap<String, Object>();
			if (MapUtil.isNullOrEmpty(reqMap)) {
				Map<String, Object> aMap = msg.toMap();
				reqMap = new HashMap<String, Object>();
				for (String key : aMap.keySet()) {
					reqMap.put(key.substring(0, 1).toUpperCase() + key.substring(1), aMap.get(key));
					reqMap.put(key.substring(0, 1).toLowerCase() + key.substring(1), aMap.get(key));
				}
			}
			param.putAll(reqMap);
			param.put("type", ServiceConstants.PARM_TYPE_DEFAULT);// 4代表dataPage登录请求
			...
			LogUtil.info("[invokeService] param=" + param);
			try {
				Map<String, Object> rule = dataService.getOneRow(serviceInfo);
				LogUtil.info("[invokeService] rule=" + rule);
				// 返回参数矫正
				...
				e.printStackTrace();
				LogUtil.error("Failed to initialize new BaseMsg object due to " + e.getMessage()+". msg="+msg);
				if (CommonUtils.toBoolean(channelService.getChannelProperty(msg.getChannelId(), "WX_DEBUG_MODE"))) {
