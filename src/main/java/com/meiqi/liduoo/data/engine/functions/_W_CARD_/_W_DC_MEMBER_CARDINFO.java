 ...
 * 4、卡券来源，0为公众平台创建的卡券数据、1是API创建的卡券数据
 ...
 * errmsg :"",
    "list": [ 
        { 
			...
			verify_user:1,//	使用人数
			active_user:1,//	激活人数
			total_user:1,//	有效会员总人数
			total_receive_user:1,//	历史领取会员卡总人数
        }
 ...
 */
public class _W_DC_MEMBER_CARDINFO extends WeChatFunction {
	public static final String NAME = _W_DC_MEMBER_CARDINFO.class.getSimpleName();
	...
	public Object eval(CalInfo calInfo, Object[] args) throws RengineException, CalculateError {
		...
		String key = appId + "@" + appSecret+ "@" + beginStr + "@" + endStr + "@" + condSource + "@"
				+ NAME;
		final boolean noCache = "nocache".equalsIgnoreCase(DataUtil.getStringValue(args[args.length - 1]));
		GetCardMemberInfoResponse summary = noCache ? null : (GetCardMemberInfoResponse) CacheUtils.getCache(key);
		if (summary == null) {
			...
			GetCardMemberInfoResponse result = api.getCardMemberCardInfo(beginDate, endDate, condSource);
