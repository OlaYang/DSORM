 ...
 * 1、微信AppId 
 * 2、微信AppSecret
 * 3、code: 当前需要修改的卡券Code。
   4、new_code：新的卡券Code
   5、card_id：卡券ID。自定义Code码卡券为必填。
 * 
 * 返回JSON：
 *  {
	"errcode":0,
 ...
 */
public class _W_CARD_CHANGECODE extends WeChatFunction {
	public static final String NAME = _W_CARD_CHANGECODE.class.getSimpleName();
	...
	public Object eval(CalInfo calInfo, Object[] args) throws RengineException, CalculateError {
		...
		final String newCode = DataUtil.getStringValue(args[3]);
		final String cardId = args.length > 4 ? DataUtil.getStringValue(args[4]) : "";

		ApiConfig config = WeChatUtils.initApiConfig(appId, appSecret);
		CardAPI api = new CardAPI(config);

		BaseResponse result = api.changeCode(code, newCode, cardId);
