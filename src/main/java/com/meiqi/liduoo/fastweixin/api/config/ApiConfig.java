...
import com.meiqi.liduoo.fastweixin.api.response.GetJsApiTicketResponse;
import com.meiqi.liduoo.fastweixin.api.response.GetTokenResponse;
import com.meiqi.liduoo.fastweixin.exception.WeixinException;
import com.meiqi.liduoo.fastweixin.handle.ApiConfigChangeHandle;
...
public final class ApiConfig extends Observable implements Serializable {
	...
	/**
	 * 这里定义token正在刷新的标识，想要达到的目标是当有一个请求来获取token，发现token已经过期（我这里的过期逻辑是比官方提供的早100秒）
	 * ，然后开始刷新token
	 * 在刷新的过程里，如果又继续来获取token，会先把旧的token返回，直到刷新结束，之后再来的请求，将获取到新的token
	 * 利用AtomicBoolean实现原理：
	 * 当请求来的时候，检查token是否已经过期（7100秒）以及标识是否已经是true（表示已经在刷新了，还没刷新完），过期则将此标识设为true，
	 * 并开始刷新token
	...
	private static Map<String, ApiConfig> instances = new ConcurrentHashMap<String, ApiConfig>();

	public static ApiConfig getInstance(String appid, String appSecret) {
		appid = appid.trim();
		appSecret = appSecret.trim();
		String key = appid + "@" + appSecret;
		ApiConfig instance = instances.get(key);
		if (instance == null) {
			instance = new ApiConfig(appid, appSecret, true);
			instances.put(key, instance);
		}
		return instance;
	}

	public static void removeInstance(String appid, String appSecret) {
		appid = appid.trim();
		appSecret = appSecret.trim();
		String key = appid + "@" + appSecret;
		instances.remove(key);
		forceRefreshToken(appid,appSecret); //清理AccessToken中控服务器缓存
	}
	private static void forceRefreshToken(String appid,String secret) {
		String url = LdConfigUtil.getConfig("access_token_server");
		Map<String, String> params = new HashMap<String, String>();
		params.put("appid", appid);
		params.put("secret", secret);
		params.put("forcerefresh", "1");
		HttpBaseKit.get(url, params);
	}
	/**
	...
	private ApiConfig(String appid, String secret) {
		this(appid, secret, true); // Frank，2015-12-14，强制为true，否则这个参数变化会导致缓存中出现冲突
	}
	...
	public String getAccessToken() {
		...
		params.put("appid", appid);
		params.put("secret", secret);
		params.put("return", "access_token");//预留，返回值类型，过段时间使用，预防服务器切换出错。2016-01-16
		String ret = HttpBaseKit.get(url, params);
		if (ret==null||ret.trim().startsWith("ERROR:")) {
			throw new WeixinException("微信公众号token获取出错，错误信息:" + ret.substring(6));
		}

		return ret.trim();
	}

	private String getAccessToken_old() {
		long now = System.currentTimeMillis();
		long time = now - this.weixinTokenStartTime;
		try {
			/*
			 * 判断优先顺序： 1.官方给出的超时时间是7200秒，这里用7100秒来做，防止出现已经过期的情况
	...
	public String getJsApiTicket() {
		...
		params.put("secret", secret);
		params.put("ticket", "1");
		params.put("return", "jsapi_ticket");//预留，返回值类型，过段时间使用，预防服务器切换出错。2016-01-16
		String ret = HttpBaseKit.get(url, params);
		if (ret==null||ret.trim().startsWith("ERROR:")) {
			throw new WeixinException("微信公众号JS ticket获取出错，错误信息:" + ret.substring(6));
		}
	...
	public String getCardApiTicket() {
		...
		params.put("appid", appid);
		params.put("secret", secret);
		params.put("wx_card_ticket", "1");
		params.put("return", "wx_card_ticket");//预留，返回值类型，过段时间使用，预防服务器切换出错。2016-01-16
		String ret = HttpBaseKit.get(url, params);
		if (ret==null||ret.trim().startsWith("ERROR:")) {
			throw new WeixinException("微信公众号CARD API ticket获取出错，错误信息:" + ret.substring(6));
		}

		return ret.trim();
	}
	private String getJsApiTicket_old() {
		if (enableJsApi) {
			long now = System.currentTimeMillis();
			try {
