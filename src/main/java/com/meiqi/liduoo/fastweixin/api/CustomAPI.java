...
public class CustomAPI extends BaseAPI {
     ...
     * @return 添加结果
     */
    public BaseResponse addCustomAccount(CustomAccount customAccount) {
        LOG.debug("添加客服帐号.....");
     ...
     * @return 修改结果
     */
    public BaseResponse updateCustomAccount(CustomAccount customAccount) {
        LOG.debug("修改客服帐号信息......");
        BeanUtil.requireNonNull(customAccount.getAccountName(), "帐号必填");
        BeanUtil.requireNonNull(customAccount.getNickName(), "昵称必填");
     ...
     * @return 删除结果
     */
    public BaseResponse deleteCustomAccount(CustomAccount customAccount) {
        LOG.debug("删除客服帐号信息......");
        BeanUtil.requireNonNull(customAccount.getAccountName(), "帐号必填");
        //BeanUtil.requireNonNull(customAccount.getNickName(), "昵称必填");
...
//        }
     ...
     * @return 设置结果
     */
    public BaseResponse uploadHeadImg(String accountName, File file) {
        LOG.debug("设置客服帐号头像.....");
     ...
     * @return
     */
    public GetCustomOnlineAccountsResponse getCustomOnlineAccountList() {
        LOG.debug("获取所有在线客服帐号信息....");
        GetCustomOnlineAccountsResponse response;
        String url = BASE_API_URL + "cgi-bin/customservice/getonlinekflist?access_token=#";
        BaseResponse r = executeGet(url);
        String resultJson = isSuccess(r.getErrcode()) ? r.getErrmsg() : r.toJsonString();
        response = JSONUtil.toBean(resultJson, GetCustomOnlineAccountsResponse.class);
        return response;
    }
